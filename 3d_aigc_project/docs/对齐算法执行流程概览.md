...
- **80% 体积重合门控**（见 [5.7](#57-80-体积重合门控-soft-gate)）
...
| `CASA_MIN_INLAY_OVERLAP_RATIO` | `0.80` | 镶嵌 shank（或 full-ring 整模）采样点落在 AI 体内的最低比例（融合下载门槛） |

### 5.6 回归测试

合成 torus 戒圈：`ai-service/tests/test_casa_alignment.py`（0.2× / 4× 尺度 + roll 角 + 80% overlap gate + prong refine）。

### 5.7 80% 体积重合门控（soft gate）

**动机：** 仅靠 shank 表面距离 / 直径比无法阻止「AI 与琥珀底座完全分离」的灾难性错位；80% 门槛在可融合与可预览之间取得平衡。

**指标 `_compute_inlay_ai_overlap_ratio`：**

1. 在镶嵌 **shank 内壁接触带** 均匀采样（默认 6000 点；有 ring frame 时用 `_sample_inlay_inner_wall_points`，排除爪位；点不足时扩大 axial_band）。
2. 对每个采样点判断是否与 AI mesh 重合：
   - 优先 `trimesh.contains`（水密 mesh）；
   - 否则 `ProximityQuery.on_surface` 距离 ≤ 容差（≈ `max(diameter×2.2%, 0.12mm)`）。
...
| CASA Phase3 末 | `_refine_alignment_inlay_overlap` 尺度 0.35–2.5× + 轴向/平面平移 + ±8° roll |
| ring_frame_fallback | 接受 pose 后立即 `_refine_alignment_inlay_overlap` |
| ICP | 若 overlap 下降 >3% 则拒绝 ICP 变换 |
| CASA 质量门 | `overlap_ratio >= 0.80` 否则 `reason=inlay_overlap_gate_failed` |
| `align_generated_to_base` 末 | ICP / bore 编辑后再次 refine；仍 < 80% → **`overlap_gate_failed`**（不硬中止） |
| `process_generated_mesh` | ≥80%：融合下载 + 分色 GLB；<80%：仍尝试分色预览，下载回退 AI 主体 + `fusion_warning` |
...
- `scale_contact`：根据镶嵌内径与 AI 外径接触关系微调（clip 0.98–1.15；prong-only 放宽至 1.25）。
...
| CASA inlay overlap < 80% | **soft gate**：仍尝试分色预览，下载回退 AI 主体 + 警告 |
...
| `CASA_MIN_INLAY_OVERLAP_RATIO` | `0.80` | 镶嵌与 AI 体重合采样比例下限（融合下载） |
| `USE_OMNI_CONDITIONING` | `true` | 有 setting_mesh 时尝试 Hunyuan3D-Omni 点云条件 |
| `OMNI_MODEL_PATH` | `./models/hunyuan3d-omni` | Omni 权重本地目录 |
